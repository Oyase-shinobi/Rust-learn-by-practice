...
// is the length of the slice. The word size is the same as usize, 
// determined by the processor architecture, e.g. 64 bits on an x86-64. 
...
fn main() {
    ...
    // slice holds the pointer and the length field which each of them is of usize = 8 bytes
    // therefore, 8 * 2 = 16
